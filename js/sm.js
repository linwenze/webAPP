    ...
    FastClick.prototype.needsClick = function(target) {
        ...
        switch (target.nodeName.toLowerCase()) {
            ...
            case 'select':
                ...
                break;
            case 'textarea':  //by zh 2016-1-29 textarea及input为text和password的标签，调用元素的点击，不用模拟的点击
                return true;
                break;
            case 'input':
                // File inputs need real clicks on iOS 6 due to a browser bug (issue #68)  
                if ((deviceIsIOS && target.type === 'file') || target.disabled || target.type === 'text' || target.type === 'password' || target.type === 'tel') {
                    return true;
                }
                break;
            case 'label':
    ...
    FastClick.prototype.needsFocus = function(target) {
        switch (target.nodeName.toLowerCase()) {
            ...
            case 'input':
                ...
                //by zh 2016-1-29 textarea及input为text和password的标签，调用元素的点击，不用模拟的点击
...
+ function($) {
    ...
    $.fn.datetimePickerSet = function(params) {
        return this.each(function() {
            if(!this) return;
            var p = $.extend(defaults, params);
            $(this).picker(p);
